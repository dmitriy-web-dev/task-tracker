...
@section('content')
    <h3 class="page-title">@lang('tasktracker.roles.title')</h3>
            ...
            <div class="row">
                <div class="col-md-6">
                    <table class="table table-bordered table-striped">
                        <tr>
                            <th>@lang('tasktracker.roles.fields.title')</th>
                ...
                <tr data-entry-id="{{ $user->id }}">
                    <td field-key='name'>{{ $user->name }}</td>
                                ...
                                <td field-key='role'>{{ $user->role->title ?? '' }}</td>
                                                                <td>
                                    @can('user_view')
                                    <a href="{{ route('admin.users.show',[$user->id]) }}" class="btn btn-xs btn-primary">@lang('tasktracker.qa_view')</a>
                                    @endcan
                                    @can('user_edit')
                                    <a href="{{ route('admin.users.edit',[$user->id]) }}" class="btn btn-xs btn-info">@lang('tasktracker.qa_edit')</a>
                                    @endcan
                                    @can('user_delete')
{!! Form::open(array(
        ...
        @else
            <tr>
                <td colspan="10">@lang('tasktracker.qa_no_entries_in_table')</td>
            </tr>
        @endif
    </tbody>
</table>
            ...
            <a href="{{ route('admin.roles.index') }}" class="btn btn-default">@lang('tasktracker.qa_back_to_list')</a>
        </div>
    </div>
@stop
